...
let rec str_to_part s =
  ...
  in
  let parsed = s |> str_to_list |> iter "" 0 [] false |> List.rev in
  List parsed

let rec part_to_expr : partial -> expr = function
  | Str x ->
      if String.get x 0 = '~' then Not (Var (String.make 1 (String.get x 1)))
      else Var x
...
let convert theorem =
  let assums, conseqs = theorem |> str_to_list |> split_sides in
